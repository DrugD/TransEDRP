...
def CTRP_IC50():
    ...
    with open(Exp_id_raw_dir, "r") as f:  # 打开文件
        Exp_data = f.read()  # 读取文件
    ...
    for line in tqdm(lines[1:]):
        ...
        exp_id, error, _, cell_line_value, drug_value, drug_id = line
        error = float(error)
    ...
    for line in tqdm(Cell_line[1:]):
        ...
        name = line[1]
        master_ccl_id = line[0]
        
        Cell_line_id2name_mp[str(master_ccl_id)] = str(name)

    # 对接Cell-line
    Exp_id2cell_line_name_mp = {}
    Exp_data = Exp_data.split('\n')
    for line in tqdm(Exp_data[1:]):
        line = line.split('\t')  # 读取一行文件，包括换行符
        ...
        Exp_id2cell_line_name_mp[str(exp_id)] = Cell_line_id2name_mp[str(cell_id)]
    ...
    # 从GDSCv2的mutpan文件中获取 cell line name 和 feature 匹配到CTRPv2中
    Miss_cell_line = []
    for key,values in tqdm(Exp_id2cell_line_name_mp.items()):
        values = str.lower(values)
        
        if cell_name_dict_re.get(values):
            cell_line2feature[key] = cell_feature[cell_dict[cell_name_dict_re.get(values)]]
            continue
        
        if cell_name_dict_re_wo_.get(values):
            cell_line2feature[key] = cell_feature[cell_dict[cell_name_dict_re_wo_.get(values)]]
            continue
        
        Miss_cell_line.append(values)
        
    print('From ',len(Exp_id2cell_line_name_mp.items()),' find ',len(Miss_cell_line))
    ...
    for key, value in tqdm(cell_line_drug_mp.items()):
        ...
        exp_id, drug_id = key.split("*")
        
        if cell_line2feature.get(exp_id) is None:
            continue
            
        if drug_dict.get(drug_id) is None:
            continue
        ...
        if ic50 <0:
            ...
            temp_data.append([
                cell_line2feature[exp_id],
                drug_smile[drug_dict[drug_id]],
                ic50
            ])
