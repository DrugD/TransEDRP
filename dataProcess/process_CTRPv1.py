...
def load_drug_smile(drug_dir):
    
    # 构建 drug dict {name:smile}  
    mol_dict = {}
    with open(drug_dir, "r") as f:  # 打开文件
        data = f.read()  # 读取文件
        
    lines = data.split('\n')

    for line in tqdm(lines[1:]):
        line = line.split('\t')  # 读取一行文件，包括换行符
        

        if len(line) != 11:
            pdb.set_trace()
            assert len(line) == 11
        mol_dict[line[0]] = line[7]
...
def CTRP_IC50():
    seed_torch(42)
    IC50_dir = "/home/lk/project/drug/dataset_DRP/v1/v10.D2.avg_pct_viability_data.txt"
    Cell_line_raw_dir = "/home/lk/project/drug/dataset_DRP/v1/v10.M2.cell_line_info_changeFor_GDSCv2.txt"
    Drug_dir = "/home/lk/project/drug/dataset_DRP/v1/v10.M1.informer_set.txt"
    
    with open(IC50_dir, "r") as f:  # 打开文件
        IC50_data = f.read()  # 读取文件
        
    with open(Cell_line_raw_dir, "r") as f:  # 打开文件
        Cell_line = f.read()  # 读取文件

    
    lines = IC50_data.split('\n')
    cell_line_drug_mp = {}
    ...
    for line in tqdm(lines[1:]):
        ...
        cell_line_name, drug_name,_ , cell_line_value, drug_value = line
        
        cell_line_value, drug_value = float(cell_line_value), float(drug_value)
        
        key = cell_line_name+'*'+drug_name
        if cell_line_drug_mp.get(key):
            cell_line_drug_mp[key]['data'].append([cell_line_value, drug_value])
        else:
            cell_line_drug_mp[key] = {'data':[]}
            cell_line_drug_mp[key]['data'].append([cell_line_value, drug_value])

    print("cell_line_drug_mp length is :", len(cell_line_drug_mp))
    
    # 计算IC50, 多项式回归
    for index,( key, value) in tqdm(enumerate(cell_line_drug_mp.items())):
        x0 = [x[1] for x in value['data']]
        y0 = [x[0] for x in value['data']]
        ...
        try:
            popt,pcov=curve_fit(lambda t,a,b,c,d: a/(d+b*np.exp(-c*t)),x0,y0)
            if (2*popt[0]-popt[3])/popt[1] <= 0:
                continue
            IC50 = -math.log(   (2*popt[0]-popt[3])/popt[1]) /popt[2]
            # popt[0]/(1+popt[1]*np.exp(-popt[2]*0.5))
            cell_line_drug_mp[key]['ic50'] = IC50
            
        except RuntimeError as e:
            # print(x0,y0,'make Error.')
            # pdb.set_trace()
            continue
    ...
    print("After cal IC50, cell_line_drug_mp length is ", len(cell_line_drug_mp))
    
    # 对接Cell-line
    Cell_line_name_mp = {}
    Cell_line = Cell_line.split('\n')
    for line in tqdm(Cell_line[1:]):
        line = line.split('\t')  # 读取一行文件，包括换行符
        ...
        name = line[0]
        synonym_name = line[1].split(";")
        Cell_line_name_mp[name] = []
        
        Cell_line_name_mp[name].append(name)
        if synonym_name[0]!='':
            Cell_line_name_mp[name] =  Cell_line_name_mp[name]+synonym_name
            # pdb.set_trace()
    ...
    drug_dict, drug_smile, smile_graph = load_drug_smile(Drug_dir)
    
    
    cell_dict_id2index = {value:key for key,value in cell_dict.items()}
    cell_name_dict_re = {value:key for key,value in cell_name_dict.items()}
    
    cell_line2feature = {}
    
    # Miss_cell_line = []
    for key,values in tqdm(Cell_line_name_mp.items()):
            
        for value in values:
            if cell_name_dict_re.get(value):
                cell_line2feature[key] = cell_feature[cell_dict[cell_name_dict_re[value]]]
                continue
            
            if cell_name_dict_re.get(value.replace(' ','-')):
                cell_line2feature[key] = cell_feature[cell_dict[cell_name_dict_re[value.replace(' ','-')]]]
                continue

        # if cell_line2feature.get(key) is None:
        #     Miss_cell_line.append(key)
    ...
    for key, value in tqdm(cell_line_drug_mp.items()):
        cell_line_name, drug_name = key.split("*")

        if cell_line2feature.get(cell_line_name) is None:
            continue
            
        if drug_dict.get(drug_name) is None:
            continue
        
        if value.get('ic50') == None:
            continue
        ...
        if ic50 <0:
            ...
            temp_data.append([
                cell_line2feature[cell_line_name],
                drug_smile[drug_dict[drug_name]],
                ic50
            ])
    ...
    train_data = Dataset_pan_mut(root='/home/lk/project/drug/dataset_DRP/CTRPv1_logPred', dataset=dataset+'_train', xd=xd_train, xt=xc_train, y=y_train, smile_graph=smile_graph)
    val_data = Dataset_pan_mut(root='/home/lk/project/drug/dataset_DRP/CTRPv1_logPred', dataset=dataset+'_val', xd=xd_val, xt=xc_val,  y=y_val, smile_graph=smile_graph)
    test_data = Dataset_pan_mut(root='/home/lk/project/drug/dataset_DRP/CTRPv1_logPred', dataset=dataset+'_test', xd=xd_test, xt=xc_test,   y=y_test, smile_graph=smile_graph)
